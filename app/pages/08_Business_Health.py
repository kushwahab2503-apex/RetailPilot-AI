...
def render_domain_card(title: str, status: str, subtitle: str = ""):
    # Map status to style class suffix
    suffix = "insufficient"
    if status == "Strong":
        suffix = "strong"
    elif status == "Stable":
        ...
        suffix = "risk"
    elif status in ["Insufficient Data", "Insufficient"]:
        suffix = "insufficient"

    st.markdown(f"""
        <div class="domain-card domain-card-{suffix}">
            <div class="card-label">
                {title}
            </div>
            <div class="card-value domain-status">
                {status}
            </div>
            <div class="card-meta">
                {subtitle}
...
if resolved_data is None:
    ...
    # Semantic light status palette — matches render_domain_card and shared badge system
    status_colors = {
        "Strong":           {"bg": "#ECFDF5", "border": "#059669", "text": "#059669", "badge": "🟢"},
        "Stable":           {"bg": "#EFF6FF", "border": "#2563EB", "text": "#2563EB", "badge": "🔵"},
        "Watch":            {"bg": "#FFFBEB", "border": "#D97706", "text": "#D97706", "badge": "🟡"},
        "Risk":             {"bg": "#FEF2F2", "border": "#DC2626", "text": "#DC2626", "badge": "🔴"},
        "Insufficient Data":{"bg": "#F1F5F9", "border": "#94A3B8", "text": "#64748B", "badge": "⚪"},
    }
    ...
    st.markdown(f"""
        ...
        ">
            <h3 style="margin: 0 0 4px 0; color: #172033; font-size: 1.15rem; font-weight: 600; border: none; padding: 0;">
                Overall Business Health Profile: <span style="color: {style['text']}; font-weight: 700;">{style['badge']} {overall_status}</span>
            </h3>
            <p style="margin: 0 0 16px 0; color: #64748B; font-size: 0.78rem;">
            ...
            <div style="display: flex; gap: 16px; flex-wrap: wrap;">
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 8px 14px; border-radius: 8px; text-align: center; min-width: 100px;">
                    <div style="font-size: 0.68rem; color: #64748B; text-transform: uppercase; font-weight: 600; letter-spacing: 0.03em;">🟢 Strong</div>
                    <div style="font-size: 1.15rem; font-weight: 700; color: #059669; margin-top: 2px;">{indicator_counts.get('Strong', 0)}</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 8px 14px; border-radius: 8px; text-align: center; min-width: 100px;">
                    <div style="font-size: 0.68rem; color: #64748B; text-transform: uppercase; font-weight: 600; letter-spacing: 0.03em;">🔵 Stable</div>
                    <div style="font-size: 1.15rem; font-weight: 700; color: #2563EB; margin-top: 2px;">{indicator_counts.get('Stable', 0)}</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 8px 14px; border-radius: 8px; text-align: center; min-width: 100px;">
                    <div style="font-size: 0.68rem; color: #64748B; text-transform: uppercase; font-weight: 600; letter-spacing: 0.03em;">🟡 Watch</div>
                    <div style="font-size: 1.15rem; font-weight: 700; color: #D97706; margin-top: 2px;">{indicator_counts.get('Watch', 0)}</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 8px 14px; border-radius: 8px; text-align: center; min-width: 100px;">
                    <div style="font-size: 0.68rem; color: #64748B; text-transform: uppercase; font-weight: 600; letter-spacing: 0.03em;">🔴 Risk</div>
                    <div style="font-size: 1.15rem; font-weight: 700; color: #DC2626; margin-top: 2px;">{indicator_counts.get('Risk', 0)}</div>
                </div>
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; padding: 8px 14px; border-radius: 8px; text-align: center; min-width: 120px;">
                    <div style="font-size: 0.68rem; color: #64748B; text-transform: uppercase; font-weight: 600; letter-spacing: 0.03em;">⚪ Insufficient</div>
                    <div style="font-size: 1.15rem; font-weight: 700; color: #64748B; margin-top: 2px;">{indicator_counts.get('Insufficient Data', 0)}</div>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
